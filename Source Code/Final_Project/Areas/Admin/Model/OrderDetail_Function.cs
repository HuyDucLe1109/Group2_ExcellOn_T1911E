...
namespace _ExcellOn_.Areas.Admin.Model
{
    public class OrderDetail_Function : OrderDetail_Interface
    {
        ...
        public List<Staff> Take_List_Staff_Free(int? serviceId, int ord_id)
        {
            ...
            List<Staff> list_Staff_free = db.Staffs.Where(x => x.Staff_OrderDetail.Count == 0 && x.ServiceId == Service_Id && x.Deleted != 1).ToList();
            ...
            foreach (var item in list_Staff_OrderDetail)
            {
                ...
                {
                    Staff free_staff = db.Staffs.Where(x => x.Id == item.Staff_Id && x.Deleted != 1).FirstOrDefault();
                    if (free_staff != null)
                    {
            ...
            {
                List<Staff> list_staff_have_inTable = db.Staffs.Where(x => x.ServiceId == Service_Id && x.Deleted != 1).ToList();
